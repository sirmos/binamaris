...
A ship's treasury has real constraints. Fuel and crew reserves cannot be
...
- **The policy engine and its 5 tests** — genuinely deterministic, no
...
- **The vessel** — Binamaris tracks the real container ship EVER GIVEN
  (IMO 9811000, MMSI 353136000) via live AIS data from aisstream.io. The
  position shown on the bridge is wherever that ship actually is right
  now.
- **The treasury figures** — modeled from realistic industry reserve
  ranges, not pulled from any real shipping company's actual accounts,
  since that data is privately held and not publicly available anywhere.
...
- **The Binance Agent OS execution** — approved actions are carried out
  through a real, funded Binance Agentic sub-account, not simulated
...
- `VESSEL_MMSI` — set to `353136000` to track EVER GIVEN, or another
  vessel's MMSI if you want to track a different real ship
...
5. Check the Audit log — every one of the above is recorded there in
   real time, not reconstructed afterward.
